/**
 * Demo script to preview Chisel Studio with sample data.
 *
 * Usage:
 *   cd packages/studio && bun run demo.ts
 *
 * Reads REDIS_URL from ../../.env.local or env. Requires Redis.
 */
import { readFileSync, existsSync } from "fs";
import { resolve } from "path";
import { createEngine, defineWorkflow, FatalError } from "chisel-engine";
import { createStudio } from "./src/index";
...
if (!process.env.REDIS_URL) {
  console.error(
    "REDIS_URL not set. Either set it in .env.local at the repo root or export it."
  );
  process.exit(1);
}
...
const syncUsers = defineWorkflow<{ source: string }>(
  ...
  async (ctx) => {
    ...
    await ctx.step("diff-local-state", async () => {
      await sleep(300);
      syncAttempt++;
      // Fail every 3rd attempt to demonstrate error states
      if (syncAttempt % 3 === 0) {
...
// Trigger a batch of email sends
...
console.log(`Open ${studio.url} in your browser to view the dashboard.\n`);

// Continuously trigger new runs every 10s to keep the feed alive
setInterval(async () => {
  const workflows = [
    () =>
