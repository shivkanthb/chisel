...
By default, terminal runs are pruned from Redis so checkpoints stay bounded:
...
You can override that with `retention`, or set `retention: false` to disable pruning entirely:
